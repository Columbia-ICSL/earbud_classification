...
def train_model(
    ...
    for ep in range(TRAINING_MAX_EPOCHS if not quick else 2):
        ...
        for i, (train_features, train_labels, train_lengths) in enumerate(train_dataloader):
            ...
            optimiser.zero_grad()
            ttf = train_features.shape
     #       print(ttf[0],ttf[1],ttf[2])
...
def predict_single_model(model, features, lengths, gpu):
    ...
    with torch.no_grad():
        ...
        #python3 run_model.py bimamba_results/ ../data/filtered earbud_results/filtered
...
class RecurrentNetworkModel(nn.Module):
    def __init__(
        self, rnn_hidden_size, rnn_num_layers, rnn_dropout, ann_hidden_size, ann_dropout, rnn_type='Mamba', dim=40
    ) -> None:
        super().__init__()
        if rnn_type == 'GRU' :
            self.rnn = nn.GRU(
        ...
        self.linear = nn.Sequential(
            nn.Linear(40, ann_hidden_size[0]),
            nn.Tanh(),
    ...
    def forward(self, x, lengths):
        ...
        return lin_out.permute(0, 2, 1)  # back to [B, C, T]
